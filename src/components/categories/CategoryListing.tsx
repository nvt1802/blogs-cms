...
const CategoryListing: React.FC<IProps> = ({
  currentPage = 1,
  data,
  onChange,
  onEditItem,
          ...
          <Card key={item?.id} className="custom-card w-full">
            <div className="flex flex-row gap-2 xs:gap-4 w-full justify-between">
              ...
              <div className="flex flex-col gap-1.5 dark:text-white w-full">
                <p className="text-base truncate font-semibold w-full">{item?.name}</p>
                <p className="inline-flex gap-2 text-sm">{item?.description}</p>
